...
try:
    ...
    # [方案一 v2：新增代码] 使用适用于TF2.x的新方法禁用图优化器
    # This helps prevent crashes on Apple Silicon (M1/M2/M3) chips.
    try:
        tf.config.optimizer.set_experimental_options({"disable_meta_optimizer": True})
        print(
            "\n[INFO] TensorFlow MetaOptimizer disabled for stability on Apple Silicon.\n"
        )
    except Exception as e:
        print(
            f"\n[WARNING] Could not disable TensorFlow MetaOptimizer. Proceeding with default settings. Error: {e}\n"
        )
...
# --- CONFIGURATION (FINAL VERSION FOR 3-MONTH TEST) ---
CONFIG = {
    "symbols_to_test": ["ETHUSDT"],
    "btc_symbol": "BTCUSDT",
    "interval": "5m",
    "backtest_start_date": "2025-07-01",  # [修改] 回测3个月
    "backtest_end_date": "2025-10-01",
    "initial_cash": 500_000,
    "commission": 0.001,
    "spread": 0.0005,
    "show_plots": True,
    "training_window_days": 365 * 1,
    "ml_training_gap_days": 30,
    "use_pretrained_models": False,  # 必须重新训练
    "enabled_modules": {
...
# --- STRATEGY PARAMETERS (THE GRAND FINALE: ML-DRIVEN) ---
STRATEGY_PARAMS = {
    # 风险、市场状态、指标周期等保持不变...
    ...
    # 趋势跟踪模块 - [简化]
    "tf_donchian_period": 8,
    "tf_ema_fast_period": 12,  # 使用12周期EMA作为基础的“安全检查”
    "tf_ema_slow_period": 24,
    "tf_adx_confirm_period": 14,
    "tf_adx_confirm_threshold": 22,
    "tf_chandelier_period": 5,
    "tf_chandelier_atr_multiplier": 3.0,
    "tf_atr_period": 14,
    "tf_stop_loss_atr_multiplier": 3.0,
    # 均值回归模块
    "mr_bb_period": 20,
    "mr_bb_std": 2.5,
    "mr_rsi_period": 14,
    ...
    "mr_stop_loss_atr_multiplier": 1.8,
    "mr_take_profit_rr": 1.5,
    "mr_risk_multiplier": 0.5,
    # MTF信号
    "mtf_period": 288,
    # 进场评分系统 - [彻底废除]
    "score_entry_threshold": 0.0,  # [关键] 不再使用TA评分作为门槛
    "score_weights_tf": {},  # [关键] 权重设为空
    # 利润奔跑模式
    "lpr_enabled": True,
    "lpr_trigger_pct": 0.012,
    "lpr_trail_atr_multiplier": 2.8,
    # 机器学习过滤器 - [核心权力]
    ...
    "ensemble_w_lstm": 0.4,
    "ensemble_w_xgb": 0.6,
    "ml_confidence_threshold": 0.88,  # [至关重要] 策略的唯一生命线！只在模型近90%确信时才行动。
    "ml_score_weight": 1.0,  # 无关紧要，因为TA评分为0
}
...
def add_all_features(df: pd.DataFrame, btc_df: pd.DataFrame = None) -> pd.DataFrame:
    ...
    if not df_1d.empty:
        sma_daily = ta.trend.SMAIndicator(
            df_1d["Close"], window=50  # Using a standard 50-day SMA
        ).sma_indicator()
        df["mtf_signal"] = (
...
def make_asymmetric_labels(
    df, look_forward=150, risk_reward_ratio=2.5, sl_atr_multiplier=1.5
):
    """
    创建非对称标签（二分类：盈利、亏损），用于机器学习模型训练。
    1: 价格先触及盈利目标 (TP)
    0: 价格先触及止损目标 (SL)
    """
    atr = ta.volatility.AverageTrueRange(
    ...
    # Scale look_forward based on interval. Base is 1m.
    # We can get interval from CONFIG
    interval_str = CONFIG.get("interval", "1m")
    try:
        # Extract the number from '1m', '5m', '15m' etc.
        interval_minutes = int("".join(filter(str.isdigit, interval_str)))
        look_forward_bars = int(look_forward / interval_minutes)
    except (ValueError, ZeroDivisionError):
        look_forward_bars = look_forward  # Default to original if format is unexpected
    ...
    for i in range(len(df) - look_forward_bars):
        ...
        # Ensure barriers are valid numbers
        up_b = upper_barrier.loc[entry_idx]
        low_b = lower_barrier.loc[entry_idx]
        if not (np.isfinite(up_b) and np.isfinite(low_b)):
            continue

        upper_touch_time = path[path.High >= up_b].index.min()
        lower_touch_time = path[path.Low <= low_b].index.min()
...
class UltimateStrategy(Strategy):
    ...
    def next(self):
        ...
        # 数据预热期
        warmup_period = max(
            self.regime_norm_period, self.tf_ema_slow_period, self.mtf_period, 50
        )
    ...
    def run_scoring_system_entry(self, price):
        ...
        if (
            ...
            if not np.isnan(seq).any():
                ...
                prob_lstm = float(
                    self.lstm.predict(np.expand_dims(scaled_seq, 0), verbose=0)[0][0]
                )
        ...
        # 2. 检查基础的“安全过滤器” (TA环境)
        is_long_safe = price > self.tf_ema_fast[-1]  # 价格在短期均线上方？
        is_short_safe = price < self.tf_ema_fast[-1]  # 价格在短期均线下方？

        # 3. 最终决策：ML模型作为唯一触发器
        # 如果模型高度看多，且基础环境安全，则做多
        if ensemble_prob > self.ml_confidence_threshold and is_long_safe:
            self.open_tf_position(price, is_long=True, confidence_factor=1.0)

        # 如果模型高度看空，且基础环境安全，则做空
        elif (1 - ensemble_prob) > self.ml_confidence_threshold and is_short_safe:
    ...
    def manage_open_position(self, price):
        ...
        # 检查是否激活LPR模式 (仅对趋势跟踪策略有效)
        if (
            self.lpr_enabled
            and self.enabled_modules.get("lpr_mode", False)
            and self.position
            and self.active_sub_strategy == "TF"
        ):
            trade = self.trades[-1]
            profit_pct = (
    ...
    def manage_mean_reversion_exit(self, price):
        """管理均值回归仓位的退出。"""
        if self.position.is_long:
            if price >= self.mr_take_profit or price <= self.mr_stop_loss:
...
if __name__ == "__main__":
    ...
    if CONFIG.get("use_pretrained_models", False):
        ...
        # We need data from even earlier to train for the first backtest period
    ...
    if CONFIG.get("use_pretrained_models", False):
        # This mode is simplified now, focusing on walk-forward
        logger.info("Running a single continuous backtest without retraining.")
        bt_start = pd.to_datetime(CONFIG["backtest_start_date"])
        bt_end = pd.to_datetime(CONFIG["backtest_end_date"])
        print(
            "#" * 80
            + f"\n单次连续回测: {bt_start.date()} to {bt_end.date()}\n"
            + "#" * 80
        )

        for symbol in CONFIG["symbols_to_test"]:
            # Logic for single backtest...
            pass
    else:
        logger.info(
        ...
        walk_forward_periods = pd.date_range(
            start=backtest_start_dt, end=backtest_end_dt, freq="1MS"
        )
        ...
        for i, period_start in enumerate(walk_forward_periods):
            ...
            training_end_dt = period_start - timedelta(
                days=CONFIG["ml_training_gap_days"]
            )

            print(
                "\n"
                + "#" * 80
                + f"\n滚动周期 {i+1}: 回测 {period_start.date()} to {period_end.date().date()}\n"
                + "#" * 80
            )
            ...
            for symbol, data in all_featured_data.items():
                ...
                if (
                    ...
                    logger.info(
                        f"[{symbol}] 准备训练数据，截止于: {training_end_dt.date()} (已应用 {CONFIG['ml_training_gap_days']}-day 隔离期)"
                    )
                    training_slice = data.loc[:training_end_dt]
