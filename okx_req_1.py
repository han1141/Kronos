# okx_live_trading_v5.0_safety_audit.py
"""
OKX REST API 轮询版 (V5.0 - 终极安全审计版)
- 核心修复 (Based on Incident 2025-11-20):
    - [!!! 新增 !!!] `ensure_position_protection` 安全审计函数。
        - 独立于策略逻辑，强制检查仓位是否处于“裸奔”状态（无止损保护）。
        - 如果检测到止损单丢失（无论是因为API超时导致ID丢失，还是交易所端订单消失），立即按最后已知安全价格重设止损。
        - 解决了 V4.9 中因 API 超时导致新止损下不进去，旧止损又已取消，导致仓位无保护从而触发熔断的致命缺陷。
    - [继承] 保留了 V4.9 的防御性取消机制和无Hurst简化逻辑。

调试说明:
- 观察日志中 "[安全审计]" 相关的条目。如果出现 "检测到仓位无保护"，说明该机制成功拦截了潜在的灾难。
"""
...
STRATEGY_PARAMS = {
    ...
    "regime_score_weight_adx": 0.60,
    "regime_score_weight_atr": 0.25,
    "regime_score_weight_rsi": 0.15,
    "regime_score_threshold": 0.4,
    "macro_trend_ema_period_4h": 30,
    ...
    if is_active:
        ...
        if new_stop_price is not None:
            ...
            if res and res.get("code") == "0" and res["data"][0]["sCode"] == "0":
                ...
                trade_flow_logger.critical(
                    f"[{symbol}] [TSL] !!! 紧急情况 !!! 旧止损已取消，但新止损下单失败！响应: {res}。仓位暂时无保护！"
                )
                # [V5.0 关键安全修复]
                # 清除本地ID，虽然这里失败了，但我们清除了ID后，
                # 下一轮循环中的 ensure_position_protection 函数会检测到 ID 为 None，
                # 从而立即触发强制止损重设，不会让仓位持续裸奔。
                trade_state["current_stop_id"] = None
                save_trade_state(trade_state)


# --- [V5.0 核心新增] 安全审计函数 ---
def ensure_position_protection(trader, symbol, strategy, pos_data):
    """
    [安全审计] 确保当前仓位有对应的止损单存在。
    逻辑:
    1. 检查本地是否有 active 的 stop_id。
    2. 如果有，检查交易所是否真的有这个订单。
    3. 如果没有 ID 或 交易所没订单 -> 立即按 current_stop_price 重设止损。
    """
    if not (trade_state := load_trade_state()):
        return

    # 仅当状态记录中有止损价格时才进行审计
    if "current_stop_price" not in trade_state:
        return

    current_id = (
        str(trade_state.get("current_stop_id"))
        if trade_state.get("current_stop_id")
        else None
    )

    # 获取交易所当前的活动策略单
    open_algos = trader.fetch_open_algo_orders(symbol)
    active_algo_ids = [str(o["algoId"]) for o in open_algos] if open_algos else []

    is_protected = False

    # 情况 A: 本地有 ID，且在交易所列表中存在 -> 安全
    if current_id and current_id in active_algo_ids:
        is_protected = True

    # 如果不安全 (本地无ID，或本地有ID但交易所没有)，则必须修复
    if not is_protected:
        trade_flow_logger.critical(
            f"[{symbol}] [安全审计] !!! 检测到仓位无保护 !!! (Local ID: {current_id}, Active on Exch: {current_id in active_algo_ids if current_id else 'N/A'})"
        )

        target_price = float(trade_state["current_stop_price"])
        pos_size = abs(float(pos_data.get("pos")))
        side = "sell" if float(pos_data.get("pos")) > 0 else "buy"

        trade_flow_logger.warning(
            f"[{symbol}] [安全审计] 正在强制恢复止损单 @ {target_price}..."
        )

        res = trader.place_trigger_order(
            symbol, side, pos_size, target_price, "Safety-Audit-Restore"
        )

        if res and res.get("code") == "0" and res["data"][0]["sCode"] == "0":
            new_id = res["data"][0]["algoId"]
            trade_flow_logger.info(
                f"[{symbol}] [安全审计] 止损已成功恢复! New ID: {new_id}"
            )
            trade_state["current_stop_id"] = new_id
            save_trade_state(trade_state)
        else:
            trade_flow_logger.error(f"[{symbol}] [安全审计] 止损恢复失败! 响应: {res}")
...
def main():
    ...
    logging.info("启动 OKX REST API 轮询交易程序 (V5.0 终极安全审计版)...")
    if not all(
        (
                    ...
                    if strategy.position is not None:
                        if current_price := trader.fetch_ticker_price(symbol):
                            ...
                            ensure_position_protection(
                                trader, symbol, strategy, current_position_data
                            )
