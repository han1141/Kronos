...
csv_name = "../601398_5min_history.csv"
...
if len(df) < lookback:
    print(f"❌ shu ju bu zu! xu yao zhi shao {lookback} hang li shi shu ju, dan wen jian zhi you {len(df)} hang.")
    sys.exit()
...
print(f"li shi shu ju de zui hou yi ge shi jian dian shi: {x_timestamp_for_forecast.iloc[-1]}")
...
if freq is None:
    freq = "min"
    print(f"⚠️ wu fa zi dong tui duan shi jian pin lv, yi mo ren shi yong '{freq}'.")
...
def plot_prediction(historical_df, prediction_df, actual_df=None):
    ...
    ax1.plot(historical_df['timestamps'], historical_df['close'], 
             label='li shi shu ju (Historical)', color='blue', linewidth=1.5)
    
    # zhi jie shi yong prediction_df zhong de shi jian chuo jin xing hui tu
    ax1.plot(prediction_df['timestamps'], prediction_df['close'], 
             label='yu ce shu ju (Prediction)', color='red', linewidth=1.5, linestyle='--')
    
    if actual_df is not None and not actual_df.empty:
        ax1.plot(actual_df['timestamps'], actual_df['close'], 
                 label='zhen shi shu ju (Actual)', color='green', linewidth=1.5, alpha=0.8)
    ...
    ax2.plot(historical_df['timestamps'], historical_df['volume'], 
             label='li shi cheng jiao liang', color='blue', alpha=0.7)
    # zhi jie shi yong prediction_df zhong de shi jian chuo jin xing hui tu
    ax2.plot(prediction_df['timestamps'], prediction_df['volume'], 
             label='yu ce cheng jiao liang', color='red', alpha=0.7, linestyle='--')
    if actual_df is not None and not actual_df.empty:
        ax2.plot(actual_df['timestamps'], actual_df['volume'],
                 label='zhen shi cheng jiao liang', color='green', alpha=0.6)
