...
def create_trend_labels(df, look_forward_steps=5, target_return=0.01):
    """
    根据未来N根K线内是否达到目标收益率来创建标签。
    - 如果未来 look_forward_steps 根K线内的最高价 >= 当前收盘价 * (1 + target_return)，则标签为 1。
    - 否则为 0。
    """
    df_copy = df.copy()
    ...
    # 找到未来 N 根K线内的最高价
    # .rolling() 是向后看的，所以我们用 .shift() 将窗口向前移动
    ...
    # 返回带有标签的DataFrame，可以只保留label列或全部返回
...
def run_backtest_and_evaluate(
    ...
    if winning_trades_drawdown:
        print("\n--- [新指标] 盈利交易在盈利前的最大回撤分析 ---")
        print(f"盈利的交易总数: {len(winning_trades_drawdown)}")
        print(f"平均回撤: {np.mean(winning_trades_drawdown):.4f}%")
        print(f"最大回撤 (最差情况): {np.max(winning_trades_drawdown):.4f}%")
        print(f"最小回撤 (最好情况): {np.min(winning_trades_drawdown):.4f}%")
    else:
        print("\n--- [新指标] 盈利交易在盈利前的最大回撤分析 ---")
